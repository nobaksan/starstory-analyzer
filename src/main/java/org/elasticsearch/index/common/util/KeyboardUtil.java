...
public class KeyboardUtil {
    ...
    private static int getSingleMedial(int index, String word) {
        if ((index + 1) <= word.length()) {
            return makeUnicodeIndex(MorphemeEnum.JUNGSUNG, word.substring(index, index+1));
        } else {
            return -1;
        }
    }
    ...
    private static int getDoubleMedial(int index, String word) {
        if ((index + 2) > word.length()) {
            ...
            return makeUnicodeIndex(MorphemeEnum.JUNGSUNG, word.substring(index, index+2));
        }
    }
    ...
    private static int getSingleFinal(int index, String word) {
        if ((index + 1) <= word.length())  {
            return makeUnicodeIndex(MorphemeEnum.JONGSUNG, word.substring(index, index+1));
        } else {
            return -1;
        }
    }
    ...
    private static int getDoubleFinal(int index, String word) {
        if ((index + 2) > word.length()) {
            ...
            return makeUnicodeIndex(MorphemeEnum.JONGSUNG, word.substring(index, index+2));
        }
    }    
